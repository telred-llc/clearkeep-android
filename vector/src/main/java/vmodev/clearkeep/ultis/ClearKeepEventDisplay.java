...
public class ClearKeepEventDisplay extends EventDisplay {
    private static final String LOG_TAG = ClearKeepEventDisplay.class.getSimpleName();

    private static final Map<String, Event> mClosingWidgetEventByStateKey = new HashMap<>();

    // constructor
    public ClearKeepEventDisplay(Context context, HtmlToolbox htmlToolbox) {
        super(context, htmlToolbox);
    }

    // constructor
    public ClearKeepEventDisplay(Context context) {
        super(context);
    }
    ...
    public CharSequence getTextualDisplay(Integer displayNameColor, Event event, RoomState roomState) {
        ...
        try {
            if (TextUtils.equals(event.getType(), WidgetsManager.WIDGET_EVENT_TYPE)) {
                ...
                senderDisplayName = roomState.getMemberName(senderDisplayName);
