                ...
                if (e.isEncrypted) {
                    ...
                    if (e.content.asJsonObject.get("ciphertext") != null) {
                        Observable.create<Long> { emmiter ->
                            val message = Message(id = e.eventId, roomId = e.roomId, userId = e.sender, messageType = e.type, encryptedContent = e.content.toString(),createdTime = e.originServerTs);
                    ...
                    roomSync.timeline.events.forEach { t: Event? ->
                        ...
                        t?.let {
                            if (it.type.compareTo("m.room.encrypted") == 0) {
                                val message = Message(id = it.eventId, roomId = it.roomId, userId = it.userId, messageType = it.type, encryptedContent = it.contentAsJsonObject.toString(),createdTime = it.originServerTs);
